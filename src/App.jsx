        ...
        <div className="fixed inset-0 z-[90] bg-black/70 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in">
            <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl max-w-lg w-full overflow-hidden border-2 border-red-500 flex flex-col max-h-[85vh]">
                ...
                <div className="p-4 overflow-y-auto custom-scrollbar flex-1 space-y-4">
                    <div><div className="flex items-center gap-2 mb-2 pb-1 border-b border-gray-200 dark:border-slate-700"><span className="bg-red-100 text-red-600 font-black px-2 py-0.5 rounded text-sm">A</span><h4 className="font-bold text-gray-700 dark:text-gray-300">重要與 A 級 ({processedGroups.A.length})</h4></div>{processedGroups.A.length > 0 ? (processedGroups.A.map((item, idx) => renderItem(item, idx))) : (<div className="text-center text-gray-400 text-xs py-2">無待辦事項</div>)}</div>
...
export default function App() {
  ...
  const handleBatchDelete = async (ids) => {
      ...
      const deletableIds = targets.filter(c => isAdmin || c.owner === currentUser.username).map(c => c.id);

      if (deletableIds.length === 0) { return alert("您沒有權限刪除這些客戶 (非您名下)。"); }
      ...
      try {
          ...
          deletableIds.forEach(id => { batch.delete(doc(db, 'artifacts', appId, 'public', 'data', 'customers', id)); });
          await batch.commit();
          alert(`成功刪除 ${deletableIds.length} 筆資料`);
      } catch (e) { console.error(e); alert("刪除失敗"); } finally { setLoading(false); }
  };
