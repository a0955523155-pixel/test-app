...
const CustomerForm = ({ onSubmit, onCancel, initialData, appSettings, companyProjects, allUsers = [] }) => {
    ...
    // ... (Effects for House Age & Effective Ping same as before) ...
    ...
    const handleLandPingChange = (val) => {
        ...
        setFormData(prev => ({ ...prev, landPing: val, unitPrice: newUnit }));
    };
                        ...
                        {isCaseMode && (
                            <div className="space-y-6 bg-orange-50 dark:bg-orange-900/10 p-4 rounded-xl border border-orange-100 dark:border-orange-900/30">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div className="md:col-span-2"><label className="block text-xs font-bold text-gray-500 mb-1"><FolderOpen className="inline w-3 h-3"/> 指定歸檔地區資料夾</label><select name="assignedRegion" value={formData.assignedRegion || ''} onChange={handleChange} className="w-full p-2 rounded-lg border dark:bg-slate-800 dark:border-slate-700 dark:text-white"><option value="">(不歸檔)</option>{projectRegions.map(r => <option key={r} value={r}>{r}</option>)}</select></div>
                                ...
                                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl border dark:border-slate-700 shadow-sm">
                                    <h3 className="text-sm font-bold text-gray-500 mb-3 flex items-center gap-2"><Calculator className="w-4 h-4"/> 價格與坪數試算 ({isRental ? '出租模式' : '買賣模式'})</h3>
                                ...
                                <div className="bg-yellow-50 dark:bg-yellow-900/10 p-4 rounded-xl border border-yellow-100 dark:border-yellow-900/30">
                                    <h3 className="text-sm font-bold text-yellow-700 mb-3 flex items-center gap-2"><Calendar className="w-4 h-4"/> 委託期間</h3>
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    <div><label className="block text-xs font-bold text-gray-500 mb-1">道路 (米)</label><input name="roadWidth" value={formData.roadWidth || ''} onChange={handleChange} className="w-full p-2 rounded-lg border dark:bg-slate-800 dark:border-slate-700 dark:text-white" /></div>
                                    <div><label className="block text-xs font-bold text-gray-500 mb-1">面寬 (米)</label><input name="faceWidth" value={formData.faceWidth || ''} onChange={handleChange} className="w-full p-2 rounded-lg border dark:bg-slate-800 dark:border-slate-700 dark:text-white" /></div>
                                    <div><label className="block text-xs font-bold text-gray-500 mb-1">深度 (米)</label><input name="depth" value={formData.depth || ''} onChange={handleChange} className="w-full p-2 rounded-lg border dark:bg-slate-800 dark:border-slate-700 dark:text-white" /></div>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="bg-gray-50 dark:bg-slate-800 p-3 rounded-lg"><label className="block text-xs font-bold text-gray-500 mb-1">土地增值稅</label><div className="flex gap-2"><input name="taxGeneral" value={formData.taxGeneral || ''} onChange={handleChange} placeholder="一般" className="w-full p-2 rounded border dark:bg-slate-900 dark:border-slate-700 text-sm" /><input name="taxSelf" value={formData.taxSelf || ''} onChange={handleChange} placeholder="自用" className="w-full p-2 rounded border dark:bg-slate-900 dark:border-slate-700 text-sm" /></div></div>
                                    <div className="bg-gray-50 dark:bg-slate-800 p-3 rounded-lg"><label className="block text-xs font-bold text-gray-500 mb-1">貸款狀況</label><div className="flex gap-2"><input name="mortgageBank" value={formData.mortgageBank || ''} onChange={handleChange} placeholder="銀行" className="w-full p-2 rounded border dark:bg-slate-900 dark:border-slate-700 text-sm" /><input name="mortgageAmount" value={formData.mortgageAmount || ''} onChange={handleChange} placeholder="金額(萬)" className="w-full p-2 rounded border dark:bg-slate-900 dark:border-slate-700 text-sm" /></div></div>
                                </div>
                                <div><label className="block text-xs font-bold text-gray-500 mb-1"><Users className="inline w-3 h-3"/> 負責專員</label><div className="flex flex-wrap gap-2 p-2 border rounded-lg dark:bg-slate-800 dark:border-slate-700">{(allUsers && allUsers.length > 0 ? allUsers : [{name:'預設專員'}]).map(u => (<button key={u.id || u.name} type="button" onClick={() => toggleAgent(u.name)} className={`px-3 py-1 rounded-full text-xs font-bold transition-colors ${formData.agents?.includes(u.name) ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-600 dark:bg-slate-700 dark:text-gray-300'}`}>{u.name}</button>))}</div></div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                                ...
                                <div className="col-span-2 md:col-span-3 bg-blue-50 dark:bg-blue-900/10 p-4 rounded-xl border border-blue-100 dark:border-blue-900/30">
                                    <div className="flex justify-between items-center mb-3">
                                        <h3 className="text-sm font-bold text-blue-700 flex items-center gap-2"><CreditCard className="w-4 h-4"/> 代書作業款項</h3>
                                    ...
                                    <div className="space-y-3">
                                        {formData.scribeDetails && formData.scribeDetails.map((item, idx) => (
                                            <div key={idx} className="flex flex-wrap gap-2 items-end border-b border-blue-200 pb-2">
                                                ...
                                                <div className="flex-1 min-w-[80px]"><label className="text-[10px] text-gray-500">方式</label><input value={item.method || ''} onChange={e=>handleScribeChange(idx,'method',e.target.value)} className="w-full p-1 text-sm border rounded" placeholder="現金/匯款" /></div>
                                        ...
                                        {(!formData.scribeDetails || formData.scribeDetails.length === 0) && <p className="text-xs text-gray-400 text-center">尚未新增款項</p>}
                                    </div>
                                </div>
                            </div>
                        )}
