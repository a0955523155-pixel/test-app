                ...
                if (Array.isArray(ads)) {
                    ads.forEach(ad => {
                        ...
                        if (adObj.endDate) {
                            ...
                            list.push({ type: '廣告', name: `${projectName} - ${adObj.name}`, date: adObj.endDate, days: diff, owner: '行銷', style: style });
                        }
                    });
                }
            });
                    ...
                    <div className="space-y-4">
                        <div className={`p-4 rounded-2xl border ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-gray-200'}`}>
                            ...
                            {expiringItems.length === 0 ? <p className="text-gray-400 text-center py-10">目前無監控項目</p> : (
                                <div className="space-y-2">{expiringItems.map((item, idx) => (<div key={idx} className="flex justify-between items-center p-3 rounded-lg bg-gray-50 dark:bg-slate-900 border border-gray-100 dark:border-slate-700 hover:shadow-md transition-shadow"><div className="flex items-center gap-3"><span className={`px-2 py-1 rounded text-xs font-bold ${item.style}`}>{item.type}</span><div className="flex flex-col"><span className="font-bold dark:text-gray-200 text-sm">{item.name}</span><span className="text-xs text-gray-400">{item.owner}</span></div></div><div className="text-right"><div className={`text-sm font-bold ${item.days < 0 ? 'text-red-600' : (item.days <= 7 ? 'text-orange-500' : 'text-green-600')}`}>{item.days < 0 ? `已過期 ${Math.abs(item.days)} 天` : `剩 ${item.days} 天`}</div><div className="text-xs text-gray-400">{item.date} 到期</div></div></div>))}</div>
                            )}
                        </div>
                    </div>
                )}
            ...
            {adManageProject && (
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className={`w-full max-w-md p-6 rounded-2xl shadow-2xl transform transition-all max-h-[85vh] overflow-y-auto ${darkMode ? 'bg-slate-900 text-white' : 'bg-white'}`}>
                        ...
                        <div className="space-y-2">{(projectAds[adManageProject] || []).length === 0 ? <p className="text-center text-gray-400 text-sm">暫無廣告</p> : (projectAds[adManageProject] || []).map((ad, idx) => { const adObj = typeof ad === 'string' ? { id: idx, name: ad, endDate: '' } : ad; let daysLeft = null; if(adObj.endDate) { daysLeft = Math.ceil((new Date(adObj.endDate) - new Date()) / 86400000); } return (<div key={adObj.id || idx} className="flex justify-between items-center p-3 rounded-lg border dark:border-slate-800 text-sm hover:border-blue-300 transition-colors bg-white dark:bg-slate-800"><div><span className="font-bold block">{adObj.name}</span>{adObj.endDate && (<span className={`text-xs ${daysLeft <= 3 ? 'text-red-500 font-bold' : 'text-gray-400'}`}>到期: {adObj.endDate} {daysLeft <= 0 ? '(已過期)' : `(剩${daysLeft}天)`}</span>)}</div><div className="flex gap-1"><button onClick={() => onEditAd(ad, adManageProject)} className="p-2 text-blue-500 hover:bg-blue-50 dark:hover:bg-slate-700 rounded-full"><Edit className="w-4 h-4"/></button><button onClick={() => onDeleteAd(ad, adManageProject)} className="p-2 text-red-500 hover:bg-red-50 dark:hover:bg-slate-700 rounded-full"><Trash2 className="w-4 h-4"/></button></div></div>); })}</div>
                    </div>
                </div>
            )}
        </div>
